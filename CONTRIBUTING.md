...
La commande télécharge la dernière version depuis le dépôt Github, la met dans le répertoire `django_village/static/django_village/dist/`, retire des fichiers pour réduire la taille du paquet Python et met à jour les sommes de contrôle d’intégrité dans le fichier `village/checksums`.
...
- mettre à jour le fichier README.rst pour indiquer la nouvelle version du VILLAGE ;
...
De la même manière, lorsqu’une PR est mergée dans la branche `main`, une tâche Github Actions ([deploy-doc.yml]([./.github/workflows/deploy-doc.yml)) met à jour la [documentation statique](https://numerique-gouv.github.io/django-village/) en faisant un export statique du site d’exemple.
