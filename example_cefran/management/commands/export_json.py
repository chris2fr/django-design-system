...
class Command(BaseCommand):
    ...
    # STATIC_ROOT = os.path.join(BASE_DIR, "docs/django_cefran")
    STATIC_ROOT = Path("docs", "django_cefran")
    ...
    def handle(self, *args, **options):
        ...
        with open("example_cefran/static/json/search_data.json", "w") as data_file:
            json.dump(output, data_file, indent=4, sort_keys=True)

    def get_page_content(self, file: str):
        filename = str(file).split("/")[-2]
        if filename == "django-cefran":
            filename = "homepage"
