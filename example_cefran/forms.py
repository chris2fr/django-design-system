...
from django_cefran.constants import COLOR_CHOICES_ILLUSTRATION
from django_cefran.forms import CefranBaseForm

# /!\ In order to use formsets
from crispy_forms.helper import FormHelper
from crispy_forms.layout import Layout, Fieldset, Field

from example_cefran.models import CefranAuthor, CefranBook
from example_cefran.utils import populate_genre_choices
...
class CefranAuthorCreateForm(ModelForm, CefranBaseForm):
    class Meta:
        model = CefranAuthor
        exclude = []
        widgets = {
...
class CefranBookCreateForm(ModelForm, CefranBaseForm):
    class Meta:
        model = CefranBook
        exclude = []
        widgets = {
    ...
    book_format = forms.ChoiceField(
        ...
        widget=forms.RadioSelect(attrs={"class": "cefran-fieldset--inline"}),
    )

    # /!\ CefranGenre is a model, but it requires to format the list of object before passing to the field
...
class CefranBookCreateFormHelper(FormHelper):
    def __init__(self, *args, **kwargs):
        super(CefranBookCreateFormHelper, self).__init__(*args, **kwargs)
        # self.form_tag = False
        self.layout = Layout(
            Fieldset(
                "Ajouter un livre",
...
CefranBookCreateFormSet = inlineformset_factory(
    CefranAuthor,
    CefranBook,
    form=CefranBookCreateForm,
    extra=1,
    exclude=[],
)


class ColorForm(CefranBaseForm):
