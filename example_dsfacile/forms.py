...
from dsfacile.constants import COLOR_CHOICES_ILLUSTRATION
from dsfacile.forms import dsfacileBaseForm

# /!\ In order to use formsets
from crispy_forms.helper import FormHelper
from crispy_forms.layout import Layout, Fieldset, Field

from example_dsfacile.models import DsfacileAuthor, DsfacileBook
from example_dsfacile.utils import populate_genre_choices


class ExampleForm(dsfacileBaseForm):
    # basic fields
...
class DsfacileAuthorCreateForm(ModelForm, dsfacileBaseForm):
    class Meta:
...
class DsfacileBookCreateForm(ModelForm, dsfacileBaseForm):
    class Meta:
        model = DsfacileBook
        exclude = []
        widgets = {
...
class DsfacileBookCreateFormHelper(FormHelper):
    def __init__(self, *args, **kwargs):
        super(DsfacileBookCreateFormHelper, self).__init__(*args, **kwargs)
        # self.form_tag = False
        self.layout = Layout(
            Fieldset(
                "Ajouter un livre",
...
DsfacileBookCreateFormSet = inlineformset_factory(
    DsfacileAuthor,
    DsfacileBook,
    form=DsfacileBookCreateForm,
    extra=1,
    exclude=[],
)


class ColorForm(dsfacileBaseForm):
