...
from dsfacile.utils import generate_summary_items

from example_dsfacile.forms import ColorForm, ExampleForm

from example_dsfacile.dsfacile_components import (
...
# Used by the module = getattr(globals()["dsfacile_tags"], f"dsfacile_{tag_name}") line
from dsfacile.templatetags import dsfacile_tags  # noqa

# /!\ In order to test formset
from django.views.generic import CreateView
from django.http import HttpResponse
from example_dsfacile.forms import (
    DsfacileAuthorCreateForm,
    DsfacileBookCreateFormSet,
    DsfacileBookCreateFormHelper,
)
from example_dsfacile.models import DsfacileAuthor
from example_dsfacile.utils import format_markdown_from_file
        ...
        + [
            (
                "see_all",
                {
                    ...
                    "url": "/django-dsfacile/components/",
                },
            )
        ]
    )
...
def index(request):
    ...
    return render(request, "example_dsfacile/index.html", payload)
...
def components_index(request):
    ...
    md = markdown.Markdown(
        extensions=[
            ...
            CodeHiliteExtension(css_class="dsfacile-code"),
        ],
    )
    ...
    return render(request, "example_dsfacile/components_index.html", payload)
...
def page_component(request, tag_name):  # NOSONAR
    ...
    if tag_name == "footer":
        ...
        module = getattr(globals()["dsfacile_tags"], f"dsfacile_{tag_name}")
        ...
        return render(request, "example_dsfacile/page_component.html", payload)
    else:
        ...
        return render(request, "example_dsfacile/not_yet.html", payload)
...
def page_component_header(request):
    ...
    return render(request, "example_dsfacile/doc_markdown.html", payload)
...
def page_component_follow(request):
    ...
    return render(request, "example_dsfacile/doc_follow.html", payload)
...
def page_form(request):
    ...
    return render(request, "example_dsfacile/page_form.html", payload)


# /!\ Example view for form and formset
class DsfacileAuthorCreateView(CreateView):
    model = DsfacileAuthor
    form_class = DsfacileAuthorCreateForm
    formset = None
    template_name = "example_dsfacile/example_form.html"
    ...
    def get(self, request, *args, **kwargs):
        ...
        book_formhelper = DsfacileBookCreateFormHelper()  # noqa: F841
    ...
    def get_formset(self, request, instance=None):
        if request.POST and instance:
            self.formset = DsfacileBookCreateFormSet(
                request.POST,
                request.FILES,
                instance=instance,
            )
    ...
    def get_context_data(self, **kwargs):
        context = super(DsfacileAuthorCreateView, self).get_context_data(**kwargs)
        ...
        book_formhelper = DsfacileBookCreateFormHelper()
        ...
        if self.request.POST:
            ...
            context["formset"] = DsfacileBookCreateFormSet(
        ...
        # /!\ Don't forget your dsfacile button
        context["btn_submit"] = {
            "label": "Soumettre",
            "onclick": "",
            "type": "submit",
    ...
    def post(self, request, *args, **kwargs):
        ...
        form_class = self.get_form_class()
        form = self.get_form(form_class)
        formset = DsfacileBookCreateFormSet(
            self.request.POST, self.request.FILES, instance=instance
        )
    ...
    def form_valid(self, form, formset):  # type: ignore
        """
        Called if all forms are valid. Creates a DsfacileAuthor instance along
...
def doc_contributing(request):
    payload = init_payload("Contribuer à Django-dsfacile")
...
def doc_install(request):
    payload = init_payload("Installation de Django-dsfacile")
...
def doc_usage(request):
    payload = init_payload("Utiliser Django-dsfacile")
...
def resource_icons(request):
    ...
    return render(request, "example_dsfacile/page_icons.html", payload)
...
def resource_pictograms(request):
    ...
    picto_root = "dsfacile/static/dsfacile/dist/artwork/pictograms/"
    ...
    return render(request, "example_dsfacile/page_pictograms.html", payload)
...
def resource_colors(request):
    ...
    return render(request, "example_dsfacile/page_colors.html", payload)
...
def search(request):
    ...
    return render(request, "example_dsfacile/search.html", payload)
